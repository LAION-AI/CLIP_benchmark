...
def get_transform_image_size(transform):
    if transform is None:
        return (224, 224)

    image_size = _normalize_image_size(getattr(transform, "image_size", None))
    ...
    for current_transform in _iter_transforms(transform):
        current_image_size = _normalize_image_size(getattr(current_transform, "image_size", None))
        if current_image_size is not None:
            return current_image_size

        current_size = _normalize_image_size(getattr(current_transform, "size", None))
        if current_size is None:
            continue

        transform_name = current_transform.__class__.__name__.lower()
        if "crop" in transform_name:
            crop_size = current_size
        elif "resize" in transform_name:
            resize_size = current_size
        elif generic_size is None:
            generic_size = current_size
