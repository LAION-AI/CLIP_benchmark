...
CAPTIONS_FILE_NAME = 'mscoco_captions-{}.json'
...
def create_english_annotation_file(root):
    ...
    for new_img, new_txt in zip(target_images, target_captions):
        ...
        original_caption = captions_section['caption']
        new_caption_2_old_caption[new_txt] = original_caption
    ...
    new_annotation_data = {
        ...
        'custom_lookup_table': new_caption_2_old_caption
    }
...
def create_translation_file(index_data, root, lang_code):
    ...
    download_path = os.path.join(GITHUB_MAIN_PATH, CAPTIONS_FILE_DOWNLOAD_NAME.format('en'))
    new_en_captions = _get_downloadable_file("multilingual_coco_en_captions.txt", download_path, False)
    ...
    translation_mapping = {}
    for en, new_lang in zip(new_en_captions, new_captions):
        if (en in local_mapping):
            old_en = local_mapping[en]
            translation_mapping[old_en] = new_lang
